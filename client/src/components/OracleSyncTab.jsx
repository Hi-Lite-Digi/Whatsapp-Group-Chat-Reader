...
function knownCaseSummary(caseItem) {
  ...
  return [
    ...
    ...(fields.availabilities || []).map(value => value === 'ready_stock' ? 'Ready stock confirmed' : value)
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '18px', flexWrap: 'wrap' }}>
          <div>
            ...
            <p style={{ color: 'var(--text-muted)', marginTop: '5px', maxWidth: '720px', lineHeight: 1.55 }}>
              Listen to selected supplier groups, require complete new-tyre quotation evidence, match the exact Oracle product, and update or create the approved record.
            </p>
          </div>
      ...
      <section className="glass-panel" style={{ overflow: 'hidden' }}>
        <div style={{ padding: '20px', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'center' }}>
          <div>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem', marginTop: '4px' }}>Only records with brand, model, size, price, quantity, and confirmed ready-stock availability can be published.</p>
          </div>
                ...
                {oracleEvents.map(event => (
                  <tr key={event.id} style={{ borderBottom: '1px solid var(--border-color)', verticalAlign: 'top' }}>
                    ...
                    <td style={{ padding: '15px 16px', color: 'var(--text-muted)', fontSize: '0.8rem' }}>
                      ...
                      <div style={{ marginTop: '4px' }}>{event.availability === 'ready_stock' ? 'Ready stock confirmed' : 'Availability unconfirmed'}</div>
                    </td>
